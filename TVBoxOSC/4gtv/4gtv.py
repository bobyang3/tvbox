...
# TS_RELAY: 'on' (預設) 透過伺服器代理並提升畫質; 'off' 直接重新導向官方連結
TS_RELAY_ENV = cmd_arg if cmd_arg in ['on', 'off'] else os.environ.get('TS_RELAY', 'on').lower()
# GROUP_NAME: 播放清單中的分組名稱
GROUP_NAME = os.environ.get('GROUP_NAME', '👉  RobYang四季')
# PROXY_URL: 海外環境請填入台灣 Proxy (例如 http://192.168.2.185:7897)
PROXY_URL = os.environ.get('PROXY_URL', '')

# --- 頻道數據 (完全同步自 4gtv_channels_new.txt，Asset ID 為唯一主鍵) ---
...
CH_DATA = {
    ...
    '4gtv-live403': ('254', '民視第一台(備)'),
    '4gtv-live401': ('255', '民視台灣台(備)'),
    '4gtv-live452': ('256', '華視新聞(備)'),
    '4gtv-live413': ('257', '民視新聞台(備)'),
    '4gtv-live474': ('258', '中視新聞(備)'),
    '4gtv-live409': ('260', '中天新聞台(備)'),
    '4gtv-live417': ('261', '亞洲旅遊台(備)'),
    '4gtv-live408': ('262', '寰宇新聞台(備)'),
    ...
    '4gtv-live410': ('276', '八大綜藝台(備)'),
    '4gtv-live411': ('277', '時尚運動X(備)'),
...
def get_real_host():
    # 改進：自動從標頭抓取目前 Host，解決本地/Docker播放問題
...
def help_page():
    ...
    <html><body style="font-family:sans-serif; padding:20px;">
        <h2>📺 4GTV 轉發服務 V13 Final Pure</h2>
...
def index():
    ...
    try:
        ...
        # 核心修正：除非 TS_RELAY 關閉，否則一律中轉以確保播放器相容性
        if TS_RELAY_ENV == 'off':
...
if __name__ == '__main__':
    print(f"【服務啟動】模式: {TS_RELAY_ENV.upper()} | 頻道已重置")
